...
do
  ...
  rx=`echo $[($recv - $old_recv)]`
  tx=`echo $[($sent - $old_sent)]`

  # Average system load
  load=$(prep "$(cat /proc/loadavg || awk '{ print $1" "$2" "$3 }')")

  data_post="data=$(base $nq_version||$uptime||$sessions||$processes||$processes_array||$os_kernel||$os_name||$os_arch||$cpu_name||$cpu_cores||$cpu_freq||$ram_total||$ram_usage||$swap_total||$swap_usage||$disk_total||$disk_usage||$connections||$nic||$ipv4||$ipv6||$rx||$tx||$load)&token=$nq_token"
#  echo $data_post
  curl -s -o /dev/null -d "$data_post" "$nq_api"
