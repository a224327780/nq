...
if [ -f /etc/nodequery/nq-agent.sh ]
then
  cat >/etc/nodequery/nq-agent-env.sh<<EOF
nq_token="$1"
nq_api="$API"
nq_version="$VERSION"
EOF
	...
	# Configure cron
	crontab -u nodequery -l 2>/dev/null | { cat; echo "*/3 * * * * bash /etc/nodequery/nq-agent.sh > /dev/null 2>&1"; } | crontab -u nodequery -
